...
get_sigcell_simple <- function(knn_sparse_mat=mutualknn30,
  ...
  permutation_score_top <- mclapply(1:permutation_times, mc.cores = mycores, function(i){
    sampled_cellid <- xx2_top %>%
      ...
      unlist %>%
      sort
    xx <- randomWalk_sparse(intM=knn_sparse_mat, queryGenes=as.numeric(sampled_cellid), gamma=rw_gamma)
  ...
  message("what propertion of seed cells over all cells: ", round(sum(topidx)*100/nrow(permutation_df_top), 2), "%") # how many propertion of seed over all cells
  ...
  message("what propertion of seed cells that are enriched cells: ",  round(sum(true_cell_top_idx & topidx)*100/sum(topidx), 2), "%") # how many propertion of seed were true cells
  message("fold of true cell over seed: ", round(sum(true_cell_top_idx)/sum(topidx), 2)) # fold of true cell over seed
